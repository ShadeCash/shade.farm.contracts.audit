...
contract ShadeStaker is ReentrancyGuard, Ownable {
    ...
    uint256 public constant lockDuration =
        rewardsDuration * lockDurationMultiplier;
    ...
    constructor() Ownable() {
        ...
        addRewardToken(
            address(WETH),
            0x380Ff6e45b4606e21B686979060Cad429FB72548
        ); // Payments FTM contract
    }

    // -------------------------------- CONFIG -----------------------------------

    // Add a new reward token to be distributed to stakers
    function addRewardToken(address rewardsToken, address distributor)
        public
        onlyOwner
    {
        require(
            rewardData[rewardsToken].lastUpdateTime == 0,
            "Taken already added"
        );
        // AUDIT Finding Id: 12
        require(
            rewardTokens.length < maxRewardsTokens,
            "Maximun number of reward tokens reached"
        );
    ...
    // -------------------------------- VIEWS -----------------------------------
    //
    function rewardPerToken(address rewardsToken)
        internal
        view
        returns (uint256)
    {
        ...
        return
            rewardData[rewardsToken].rewardPerTokenStored +
            (((lastTimeRewardApplicable(rewardsToken) -
                rewardData[rewardsToken].lastUpdateTime) *
                rewardData[rewardsToken].rewardRate *
                1e18) / totalSupply);
    }

    //
    function earned(address user, address rewardsToken)
        internal
        view
        returns (uint256)
    {
        ...
        return
            (balances[user] *
                (rewardPerToken(rewardsToken) -
                    userRewardPerTokenPaid[user][rewardsToken])) /
            1e18 +
            rewards[user][rewardsToken];
    }

    //
    function lastTimeRewardApplicable(address rewardsToken)
        internal
        view
        returns (uint256)
    {
        return
            block.timestamp < rewardData[rewardsToken].periodFinish
                ? block.timestamp
                : rewardData[rewardsToken].periodFinish;
    }

    //
    function claimRewardForDuration(address rewardsToken)
        internal
        view
        returns (uint256)
    {
    ...
    function claimableRewards(address account)
        public
        view
        returns (RewardData[] memory rewardsAvailable)
    {
        uint256 length = rewardTokens.length;
        rewardsAvailable = new RewardData[](length);
        for (uint256 i = 0; i < length; i++) {
            rewardsAvailable[i].token = rewardTokens[i];
            rewardsAvailable[i].amount = earned(
                account,
                rewardsAvailable[i].token
            );
        }
    ...
    {
        for (
            uint256 i = startIndex[account];
            i < userLocksLength[account];
            i++
        ) {
        ...
        )
    {
        ...
        for (uint256 i; i < rewardTokens.length; i++) {
            _claimRewardForDuration[i] = claimRewardForDuration(
                rewardTokens[i]
            );
            _rewardPerToken[i] = rewardPerToken(rewardTokens[i]);
            _rewardBalances[i] = IERC20(rewardTokens[i]).balanceOf(
                address(this)
            );
        }
        ...
        )
    {
        ...
        _userLocks = new LockedBalance[](
            userLocksLength[account] - startIndex[account]
        );
        uint256 idx;
        for (
            uint256 i = startIndex[account];
    ...
    // -------------------------------- MUTATIVE FUNCTIONS -----------------------------------
    function stakeFrom(address account, uint256 amount)
        external
        returns (bool)
    {
        require(
            lockStakers[msg.sender],
            "Sender not allowed to stake with lock"
        );
    ...
    function _stake(
        ...
        if (lock) {
            uint256 unlockTime = ((block.timestamp / rewardsDuration) *
                rewardsDuration) + lockDuration;
            uint256 locksLength = userLocksLength[account];

            if (
                locksLength == 0 ||
                userLocks[account][locksLength - 1].unlockTime < unlockTime
            ) {
    ...
    function withdraw(uint256 amount) public nonReentrant {
        ...
        if (amount > unlocked) {
            ...
            for (
                uint256 i = startIndex[msg.sender];
                i < userLocksLength[msg.sender];
                i++
            ) {
    ...
    function notifyRewardAmount(address rewardsToken, uint256 reward) external {
        require(
            rewardDistributors[rewardsToken][msg.sender],
            "Only distributor allowed to send rewards"
        );
    ...
    function notifyRewardAmountFTM() public payable {
        require(
            rewardDistributors[address(WETH)][msg.sender],
            "Only distributor allowed to send FTM"
        );
    ...
    function _updateUserLocks(address account) internal {
        ...
        while (
            userLocks[account][startIndex[account]].unlockTime <=
            block.timestamp &&
    ...
    function _sendTokensAndPenalty(uint256 tokensAmount, uint256 penaltyAmount)
        internal
    {
        if (penaltyAmount != 0 && penaltyReceiver != address(0)) {
            ...
            if (penaltyReceiver.code.length > 0) {
                ...
                try
                    IPenaltyReceiver(penaltyReceiver).notifyReward(
                        penaltyAmount
                    )
                {} catch {}
            }
    ...
    function _notifyReward(address rewardsToken, uint256 reward) internal {
        if (block.timestamp >= rewardData[rewardsToken].periodFinish) {
            ...
            uint256 remaining = rewardData[rewardsToken].periodFinish -
                block.timestamp;
