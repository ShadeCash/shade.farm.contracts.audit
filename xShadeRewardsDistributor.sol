...
interface IXSHADE {
    ...
    function userPointHistory(address account, uint256 point)
        external
        view
...
contract xShadeRewardsDistributor is ReentrancyGuard, Ownable {
    ...
    // -------------------------------- VIEWS -----------------------------------
    //
    function _findTimestampEpoch(uint256 timeStamp)
        internal
        view
        returns (uint256)
    {
    ...
    //
    function xShadeUserBalanceAt(address user, uint256 timeStamp)
        external
        view
        returns (uint256)
    {
        ...
        return
            uint256(
                SignedMath.max(
                    point.bias -
                        (point.slope * int256(timeStamp - point.timeStamp)),
                    0
                )
            );
    }
    ...
    function _checkpointToken() internal {
        ...
        uint256 rewardsToDistribute = rewardTokenBalance -
            rewardTokenLastBalance;
            ...
            if (block.timestamp < nextWeek) {
                if (sinceLast == 0 && block.timestamp == _lastRewardsTime) {
                    ...
                    rewardsPerWeek[thisWeek] +=
                        (rewardsToDistribute *
                            (block.timestamp - _lastRewardsTime)) /
                        sinceLast;
                }
                break;
            } else {
                if (sinceLast == 0 && nextWeek == _lastRewardsTime) {
                    ...
                    rewardsPerWeek[thisWeek] +=
                        (rewardsToDistribute * (nextWeek - _lastRewardsTime)) /
                        sinceLast;
                }
            }
    ...
    function checkpointToken() external {
        ...
        require(
            msg.sender == owner() ||
                (allowCheckpointToken &&
                    (block.timestamp >
                        lastRewardsTime + TOKEN_CHECKPOINT_DEADLINE))
        );
        _checkpointToken();
    }
    ...
    function _checkpointTotalSupply() internal {
        ...
        for (uint256 i = 0; i < 20; i++) {
            if (_timeCursor >= roundedTimestamp) {
                ...
                xShadeSupply[_timeCursor] = uint256(
                    SignedMath.max(point.bias - (point.slope * delta), 0)
                );
            }
            _timeCursor += WEEK;
        }
    ...
    //
    function _claim(address addr, uint256 _lastRewardsTime)
        internal
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < 50; i++) {
            ...
            if (
                weekCursor >= userPoint.timeStamp && userEpoch <= maxUserEpoch
            ) {
                userEpoch += 1;
                ...
                uint256 balanceOf = uint256(
                    SignedMath.max(
                        oldUserPoint.bias - (delta * oldUserPoint.slope),
                        0
                    )
                );
                ...
                if (balanceOf > 0) {
                    if (xShadeSupply[weekCursor] != 0) {
                        rewardsToDistribute +=
                            (balanceOf * rewardsPerWeek[weekCursor]) /
                            xShadeSupply[weekCursor];
                    }
                }
                weekCursor += WEEK;
            }
    ...
    function claim(address addr) public nonReentrant returns (uint256) {
        ...
        if (
            allowCheckpointToken &&
            (block.timestamp > _lastRewardsTime + TOKEN_CHECKPOINT_DEADLINE)
        ) {
            _checkpointToken();
    ...
    //
    function claimMany(address[20] memory accounts)
        external
        nonReentrant
        returns (bool)
    {
    ...
    function notifyReward(uint256 amount) public payable {
        ...
        if (
            allowCheckpointToken &&
            (block.timestamp > lastRewardsTime + TOKEN_CHECKPOINT_DEADLINE)
        ) {
            _checkpointToken();
        }
    }
    ...
    event Claimed(
        address indexed accouunt,
        uint256 amount,
        uint256 indexed claimEpoch,
        uint256 indexed maxEpoch
    );
    event Expired();
